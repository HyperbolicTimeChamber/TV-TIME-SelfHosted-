...
export async function addToTrackedBy(showId: string, uid: string): Promise<void> {
...
export async function removeFromTrackedBy(showId: string, uid: string): Promise<number> {
	const db = getFirestore();
	const showRef = db.doc(`shows/${showId}`);
