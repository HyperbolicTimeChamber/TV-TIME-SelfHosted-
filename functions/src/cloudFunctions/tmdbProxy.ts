...
import { TMDB_BASE, TmdbProxyAction } from "../shared/enums";
import { ProxyRequest } from "../shared/types";
...
export const tmdbProxy = onCall(
	...
	async (request) => {
		...
		switch (action) {
			...
			case TmdbProxyAction.SUGGESTIONS: {
				const { query } = data;
				if (!query) throw new HttpsError("invalid-argument", "query required");
			...
			case TmdbProxyAction.SHOW_DETAILS: {
				const { tmdbId, mediaType = "tv" } = data;
				if (!tmdbId) throw new HttpsError("invalid-argument", "tmdbId required");
			...
			case TmdbProxyAction.SEASON_DETAILS: {
				const { tmdbId, seasonNumber } = data;
				if (!tmdbId || seasonNumber === undefined)
					throw new HttpsError("invalid-argument", "tmdbId and seasonNumber required");
				const res = await client.get(`/tv/${tmdbId}/season/${seasonNumber}`);
				return res.data;
			}

			case TmdbProxyAction.DISCOVER_TV: {
			...
			case TmdbProxyAction.DISCOVER_MOVIES: {
				const { startDate, endDate } = data;
				if (!startDate || !endDate)
					throw new HttpsError("invalid-argument", "startDate and endDate required");
				const movies: Array<{
			...
			case TmdbProxyAction.FIND_BY_TVDB_ID: {
				const { tvdbId } = data;
				if (!tvdbId) throw new HttpsError("invalid-argument", "tvdbId required");
