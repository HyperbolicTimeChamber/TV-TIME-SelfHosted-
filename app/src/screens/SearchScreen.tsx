...
export default function SearchScreen() {
	...
	const route =
		useRoute<
			RouteProp<SearchStackParamList, Route.SEARCH_MAIN | Route.SEARCH_RESULTS>
		>();
	...
	// Restore tab bar when trending (SearchMain) is focused
	useFocusEffect(
		useCallback(() => {
			if (!submittedQuery) {
				...
				{!submittedQuery && (
					<TouchableOpacity
						...
						<View style={styles.searchRow}>
							...
							/>
							<Text
								style={[styles.searchInput, { color: colors.textMuted }]}
								numberOfLines={1}>
								Search shows & movies
							</Text>
						</View>
