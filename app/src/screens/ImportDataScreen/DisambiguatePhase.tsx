	...
	return (
		<View style={[styles.container, { paddingTop: insetTop + spacing.lg }]}>
			...
			<AnimatedModal visible={showInfo} onClose={() => setShowInfo(false)}>
				<View style={styles.modalContent}>
					...
					<Text style={styles.modalBody}>
						Some shows or movies from your TV Time export matched multiple results on TMDB. This can
						happen when names are shared across different shows, movies, remakes, or regional
						versions.
					</Text>
					<Text style={styles.modalBody}>
						Pick the correct match for each one so your watch history imports accurately. You can
						also skip any you don't want to import.
					</Text>
			...
			<View style={styles.disambigHeader}>
				...
				<TouchableOpacity onPress={() => setShowInfo(true)} style={{ flexShrink: 0 }}>
					<Text style={styles.infoButton}>?</Text>
				</TouchableOpacity>
			</View>
			<View style={styles.expectedRow}>
				<Text style={styles.expectedLabel}>Type: </Text>
				<View style={[styles.typeBadge, current.mediaType === "movie" && styles.typeBadgeMovie]}>
					<Text style={styles.typeBadgeText}>{current.mediaType === "tv" ? "TV" : "MOVIE"}</Text>
				</View>
				<Text style={styles.expectedHint}> · Tap to select, long press for details</Text>
			</View>
